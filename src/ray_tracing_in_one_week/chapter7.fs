...
vec3 WorldTrace(Ray ray, int depth)
{
    ...
    {
        --depth;
        if(WorldHit(ray, 0.001, T_MAX, hitRecord))
        {
            vec3 attenuation;
            Ray scatterRay;
...
World WorldConstructor()
{
	...
	world.objects[1] = SphereConstructor(vec3( 0.0, -100.5, -1.0), 100.0, MAT_LAMBERTIAN, 1);
...
void InitScene()
{
	...
	lambertMaterials[1] = LambertianConstructor(vec3(0.8, 0.8, 0.0));
